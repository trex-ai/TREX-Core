...
class Client(BaseMQTTClient):
    def __init__(self, host, port, market_configs):
        ...
        market_configs = dict(market_configs)
        ...
        try:
            ...
            market = importlib.import_module(
                "TREX_Core.markets." + market_configs["type"]
            ).Market

        self.market = market(
            client=self.client, **market_configs, grid_params=grid_params
        )
    ...
    def on_connect(self, client, _flags, _rc, _properties):
        ...
        log.info("Connected market", market_id=self.market.market_id)

    def on_disconnect(self, _client, _packet, _exc=None):
        # self.market.server_online = False
        log.info("Market disconnected", market_id=self.market.market_id)

    async def on_bid(self, message):
        bid = json.loads(message["payload"])
        try:
            entry_id, participant_id, participant_sid = await self.market.submit_bid(
                bid
            )
            self.client.publish(
                f"{self.market.market_id}/{participant_id}/bid_ack",
                entry_id,
    ...
    async def on_ask(self, message):
        ...
        try:
            entry_id, participant_id, participant_sid = await self.market.submit_ask(
                ask
            )
            self.client.publish(
                f"{self.market.market_id}/{participant_id}/ask_ack",
                entry_id,
                user_property=[("to", participant_sid)],
                qos=1,
            )
    ...
    async def on_participant_connected(self, message):
        if message["payload"]:
            ...
            market_id, market_sid, timezone = await self.market.participant_connected(
                client_data
            )
            self.client.publish(
                f"{self.market.market_id}/{client_data['id']}/market_info",
                {
                    "id": market_id,
                    "sid": market_sid,
                    "timezone": timezone,
    ...
    async def on_is_market_online(self, _message):
        self.client.publish(
            f"{self.market.market_id}/simulation/market_online",
    ...
    async def on_start_round(self, message):
        ...
        step_task = asyncio.create_task(
            self.market.step(payload["duration"], sim_params=payload)
        )
    ...
    def on_round_done(self, task):
        ...
        try:
            ...
            log.error("step() crashed", exc_info=exc)
        self.client.publish(
            f"{self.market.market_id}/simulation/end_round",
            self.market.market_id,
            qos=1,
        )
    ...
    async def on_end_episode(self, _message):
        await self.market.ensure_transactions_complete()
        await self.market.reset_market()
        self.client.publish(
            f"{self.market.market_id}/simulation/market_ready",
            self.market.market_id,
            qos=1,
        )
