# battery mode (precharge (up to 10% capacity), Current mode (10 to 80%),
# CV (80 to 100%))
...
# TODO: simplify scheduling to the net energy activity of the battery (assuming
# constant power, # within max bounds)
# TODO: add functions to check schedule
# TODO: at some point make sure all user submitted time intervals are evenly divisible
# by round duration
...
log.info("Importing battery model")

type TimeInterval = tuple[int, int]


class ScheduleEntry(TypedDict):
    energy_potential: tuple[int, int]
    projected_energy_end: float
    projected_soc_end: float
    energy_scheduled: int


type ScheduleMap = dict[TimeInterval, ScheduleEntry]


class Storage:
    """This is an idealized battery model to emulate a Li-ion or Li-Po battery

    Battery has an internal, schedule based controller. Total energy to be charged or
    discharged for a time interval is given to the controller ahead of time.
    Assumes contant voltage and current for charge/discharge control.
    ...
    # value of key is a list. first element in list is charge. second element in
    # list is discharge
    ...
    # internally it will charge less than metered and discharge more than metered
    # due to non-ideal efficiency
    ...
    def __init__(self, capacity=7000, power=3300, efficiency=0.95, monthly_sdr=0.05):
        ...
        self.__info = {
            ...
            "efficiency": efficiency,  # charges less than meter readings,
            # and discharges more than meter readings
        ...
        self.__schedule: dict[TimeInterval, int] = {}
        self.__last_round_processed: TimeInterval | None = None
        self.last_activity = 0

    # Collect status of battery charge at start and end of turn/ current
    # scheduled battery charge or discharge
    async def __status(self, **kwargs):
        current_round = cast(TimeInterval, self.timing["current_round"])
        current_energy_activity = 0
        if current_round in self.__schedule:
            # get scheduled energy activity (should be happening right now)
            current_energy_activity = self.__schedule[current_round]
        if self.__last_round_processed != current_round:
            # Predict soc at end time step if round has not been processed yet
            soc_start = self.__info["remaining_energy"]
            soc_end = max(
                0,
                soc_start
                + current_energy_activity
                - self.__info["self_discharge_rate_min"],
            )
        else:
    ...
    def __time_interval_is_valid(self, time_interval: TimeInterval) -> bool:
        # print(self.timing)
        duration = int(self.timing["duration"])
    ...
    async def check_schedule(self, time_interval: TimeInterval) -> ScheduleMap | None:
        if not self.__time_interval_is_valid(time_interval):
            log.warning("Invalid interval", time_interval=time_interval)
            return None

        current_round_start = self.timing["current_round"][0]
        if current_round_start is None:
            return None

        current_round_start = int(current_round_start)
        round_duration = int(self.timing["duration"])
        meter_potential = self.__info["power_rating"] * (round_duration / 3600)
        schedule: ScheduleMap = {}

        # charge cap and discharge cap are at the meter
        elapse_blocks = int((time_interval[1] - current_round_start) / round_duration)
        elapse_intervals = [
            (
                current_round_start + s * round_duration,
                current_round_start + (s + 1) * round_duration,
            )
            for s in range(elapse_blocks)
        ]
        request_intervals = elapse_intervals[
            elapse_intervals.index(
                (time_interval[0], time_interval[0] + round_duration)
            ) :
        ]
        ...
        for idx in range(request_blocks):
            elapse_keys = elapse_intervals[
                : elapse_intervals.index(request_intervals[idx]) + 1
            ]
            elapsed_and_scheduled = {
                key: self.__schedule[key]
                for key in elapse_keys
                if key in self.__schedule
            }
            total_scheduled_use = sum(elapsed_and_scheduled.values())
            projected_self_discharge = (
                len(elapse_keys) * self.__info["self_discharge_rate_min"]
            )
            projected_energy = max(
                0,
                min(
                    self.__info["capacity"],
                    soc_start + total_scheduled_use - projected_self_discharge,
                ),
            )
            charge_cap = (self.__info["capacity"] - projected_energy) / self.__info[
                "efficiency"
            ]  # at the meter
            ...
            scheduled_step = self.__schedule.get(request_intervals[idx], 0)
            ...
            schedule[request_intervals[idx]] = {
                ...
                "projected_soc_end": round(
                    projected_energy / self.__info["capacity"], 2
                ),
    ...
    async def schedule_energy(
        self, energy: int, time_interval: TimeInterval
    ) -> bool | int:
        ...
        energy_potential = schedule[time_interval]["energy_potential"]
        if energy > 0:
            scheduled_energy = min(energy, energy_potential[1])
        elif energy < 0:
            scheduled_energy = max(energy, energy_potential[0])
        else:
            scheduled_energy = 0

        self.__schedule[time_interval] = scheduled_energy
    ...
    async def __process_schedule(self):
        ...
        current_round = cast(TimeInterval, self.timing["current_round"])
        last_round = cast(TimeInterval, self.timing["last_round"])
        if self.__last_round_processed == current_round:
            return False
        ...
        self.__schedule.pop(last_round, None)
        self.__last_round_processed = current_round
