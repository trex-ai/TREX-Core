...
class Client:
    ...
    def on_connect(self, client, flags, rc, properties):
        ...
        client.subscribe("/".join([market_id, 'start_round']), qos=2)
        client.subscribe("/".join([market_id, participant_id]), qos=0)
        client.subscribe("/".join([market_id, participant_id, 'market_info']), qos=0)
        client.subscribe("/".join([market_id, participant_id, 'ask_ack']), qos=2)
        client.subscribe("/".join([market_id, participant_id, 'bid_ack']), qos=2)
        client.subscribe("/".join([market_id, participant_id, 'settled']), qos=2)
        client.subscribe("/".join([market_id, participant_id, 'extra_transaction']), qos=2)
        ...
        client.subscribe("/".join([market_id, 'simulation', 'end_simulation']), qos=2)
