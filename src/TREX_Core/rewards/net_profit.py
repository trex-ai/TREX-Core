from TREX_Core.rewards.utils import process_ledger


class Reward:
    def __init__(self, timing=None, ledger=None, market_info=None, **_kwargs):
    ...
    async def calculate(
        ...
        if (
            market_transactions is None
            and grid_transactions is None
            and financial_transactions is None
        ):
            (
                market_transactions,
                grid_transactions,
        ...
        grid_cost = (
            grid_transactions[0] * grid_transactions[1] if grid_transactions else 0
        )
        grid_profit = (
            grid_transactions[2] * grid_transactions[3] if grid_transactions else 0
        )
        ...
        return float(total_profit - total_cost) / 1000
