...
class Trader:
    """An intelligent trader that balances economic optimization
    with battery management.

    This trader uses value-based decision making, adaptive pricing,
    and battery state awareness to optimize market participation and
    energy use. It works with the existing TREX infrastructure
    ...
    - Manages battery charging/discharging to balance market
    ...
    def __init__(self, **kwargs):
        ...
        self.min_reserve_soc = kwargs.get(
            "min_reserve_soc", 0.2
        )  # Minimum state of charge
        self.target_peak_soc = kwargs.get(
            "target_peak_soc", 0.9
        )  # Target SoC during peak hours
        self.target_offpeak_soc = kwargs.get(
            "target_offpeak_soc", 0.5
        )  # Target SoC during off-peak
    ...
    def _is_peak_hour(self, timestamp):
        ...
        hour = utils.timestamp_to_local(
            timestamp, self.__participant["timing"]["timezone"]
        ).hour
    ...
    def _calculate_energy_value(self, timestamp, battery_soc):
        ...
        time_key = utils.timestamp_to_local(
            timestamp, self.__participant["timing"]["timezone"]
        ).hour
        ...
        return min(
            self.max_ask_price, max(self.min_ask_price, base_value * battery_factor)
        )
    ...
    def _determine_prices(self, energy_value, is_buying):
        ...
        if is_buying:
            ...
            price = max(
                self.min_bid_price, energy_value * 0.85 * self.price_adjustment_factor
            )
            return min(self.max_bid_price, price)
        # Ask price: higher than energy value (willing to sell above value)
        price = max(
            self.min_ask_price, energy_value * 1.15 * self.price_adjustment_factor
        )
        return min(self.max_ask_price, price)

    def _determine_battery_strategy(
        self,
        current_soc,
        timestamp,
        residual_load,
        ...
        if residual_load > 0:  # We need energy
            ...
            return 0
        if residual_gen > 0:  # We have excess energy
            if soc_gap > 0:
    ...
    def _update_success_rate(self, success):
        ...
        if self.attempt_count >= 10:
            ...
            if success_rate < 0.5:
                ...
                self.price_adjustment_factor = min(
                    1.5, self.price_adjustment_factor * 1.05
                )
            elif success_rate > 0.8:
                # High success rate: we can be more aggressive with prices
                self.price_adjustment_factor = max(
                    0.8, self.price_adjustment_factor * 0.98
                )
    ...
    async def _process_last_settlement(self, actions):
        """Process last-period settlement results for future decisions."""
        last_settle = self.__participant["timing"]["last_settle"]

        if last_settle in self.action_scenario_history:
            last_settle_info = await self.__participant["ledger"].get_settled_info(
                last_settle
            )
            ...
            if "bids" in last_settle_info and last_settle_info["bids"]["quantity"] > 0:
                self._update_price_history(
                    last_settle[1], last_settle_info["bids"]["price"]
                )
                self._update_success_rate(True)
            elif (
                "asks" in last_settle_info and last_settle_info["asks"]["quantity"] > 0
            ):
                self._update_price_history(
                    last_settle[1], last_settle_info["asks"]["price"]
                )
                self._update_success_rate(True)
            else:
            ...
            if scenario == "buy_and_charge":
                ...
                residual_load = self.action_scenario_history[last_settle][
                    "residual_load"
                ]
                ...
                charge_amount = max(
                    0, min(max_charge, settled_quantity - residual_load)
                )
                if charge_amount > 0:
                ...
                max_discharge = self.action_scenario_history[last_settle][
                    "max_discharge"
                ]
                ...
                if settled_quantity > 0:
                    actions["bess"] = {
                        str(last_settle): -min(abs(max_discharge), settled_quantity)
                    }
                ...
                actions["bess"] = {
                    str(last_settle): -min(abs(max_discharge), residual_load)
                }
    ...
    async def act(self, **_kwargs):
        ...
        if "storage" in self.__participant:
            ...
            current_soc = (
                storage_info["state_of_charge"] / 100.0
            ...
            max_discharge = storage_schedule[next_settle]["energy_potential"][
                0
            ]  # Negative value
            ...
            if residual_load > 0:
                ...
                actions["bids"] = {
                    str(next_settle): {"quantity": residual_load, "price": bid_price}
                }
                ...
                actions["asks"] = {
                    "solar": {  # Assuming excess is from solar
                        str(next_settle): {"quantity": residual_gen, "price": ask_price}
                    }
                }
        ...
        # No battery - simpler logic
            ...
            # Need to buy energy
            bid_price = self._determine_prices(
                0.15, True
            )  # Default value without battery
            actions["bids"] = {
                str(next_settle): {"quantity": residual_load, "price": bid_price}
            }
        elif residual_gen > 0:
            # Have excess energy to sell
            ask_price = self._determine_prices(
                0.15, False
            )  # Default value without battery
            actions["asks"] = {
                "solar": {
