...
# where the collection of experience and the learning on that experience
# happen in sequence and not in parallel
# as such, only one client is needed


# Fixme: the name that is used in the makefile here does not read from the config file?
def cli(configs):
    path_parts = __file__.split("runner")
    script_path = os.path.join(path_parts[0], "appo", "async_learner.py")
    if not os.path.exists(script_path):
        cwd = os.getcwd()
        script_path = os.path.join(cwd, "clients", "appo", "async_learner.py")
