...
def timestr_to_timestamp(time_string: str, timezone: str) -> int:
...
def process_profile(row, gen_scale=1, load_scale=1):
    ...
    Whereas tstamp is the UNIX timestamp, grid is the net load, and solar and
    solar+ are the solar generation.
    The only difference between solar and solar+ is that sometimes solar values
    can be slightly negative. The cause of this phenomenon is unknown, but the
    suspected cause may be due to reverse current flow in some parts of the
    measuring setup. Because of this, solar+ is used for calculations.

    Parameters
    ----------
    row : dict
        Must be a dict, or dict-compatible format consisting of at minimum
        tstamp, grid, and solar+
        tstamp should be a UNIX timestamp
        grid, solar, and solar+ should be in units of Wh
        the row indicates energy readings between the last tstamp and the end
        of the current tstamp
    gen_scale : float
    ...
    generation:int, consumption:int
        ...
        Integer measures should also be compatible with pulse-watt readings
        from real meters, making transition to real system relatively easy and
        seamless
    """
