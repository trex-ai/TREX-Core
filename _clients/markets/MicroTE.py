...
class Market:
    ...
    async def __settle(self, bid:dict, ask:dict, time_delivery:tuple, settlement_price=None, locking=False):
        ...
        if locking:
            await self.__client.emit('send_settlement', message, namespace='/market', callback=self.__settle_confirm_lock)
        else:
                ...
                if relevant_settlements:
                    for commit_id in relevant_settlements.keys():
                        ...
                        if not settled_quantity:
                            continue
                        residual_generation = self.__participants[seller]['meter'][time_delivery]['generation'][energy_source]
                        residual_consumption = self.__participants[buyer]['meter'][time_delivery]['consumption']['other']['external']
        ...
        for participant_id in self.__participants:
            ...
            for load in self.__participants[participant_id]['meter'][time_delivery]['consumption']:
                for source in self.__participants[participant_id]['meter'][time_delivery]['consumption'][load]:
                    if source in self.__participants[participant_id]['meter'][time_delivery]['generation']:
                        ...
                        quantity = self.__participants[participant_id]['meter'][time_delivery]['consumption'][load][source]
            ...
            for source in self.__participants[participant_id]['meter'][time_delivery]['generation']:
                ...
                if residual_generation > 0:
                    ...
                    transactions.append(transaction_record.copy())
                    self.__participants[participant_id]['meter'][time_delivery]['generation'][source] -= residual_generation
                    extra_transactions['grid']['sell'].append(transaction_record.copy())
            # buy residual consumption (other) from grid
            residual_consumption = self.__participants[participant_id]['meter'][time_delivery]['consumption']['other']['external']
            if residual_consumption > 0:
                ...
                transactions.append(transaction_record.copy())
                self.__participants[participant_id]['meter'][time_delivery]['consumption']['other']['external'] -= residual_consumption
